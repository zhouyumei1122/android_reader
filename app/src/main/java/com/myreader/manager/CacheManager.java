...
import com.myreader.util.ACache;
...
public class CacheManager {
    ...
    public List<BookLists.BookListsBean> getCollectionList() {
        List<BookLists.BookListsBean> list = (ArrayList<BookLists.BookListsBean>) ACache.get(
                BaseApplication.getInstance()).getAsObject(getCollectionKey());
        ...
        for (BookLists.BookListsBean bean : list) {
            if (bean != null) {
                if (TextUtils.equals(bean._id, bookListId)) {
                    list.remove(bean);
                    ACache.get(BaseApplication.getInstance()).put(getCollectionKey(), (Serializable) list);
                    break;
                }
            }
        }
    ...
    public void addCollection(BookLists.BookListsBean bean) {
        ...
        list.add(bean);
        ACache.get(BaseApplication.getInstance().getApplicationContext()).put(getCollectionKey(), (Serializable) list);
    ...
    public synchronized String getCacheSize() {
        ...
        try {
            ...
            if (FileUtils.isSdCardAvailable()) {
                String extCacheDir = BaseApplication.getInstance().getApplicationContext().getExternalCacheDir().getPath();
    ...
    public synchronized void clearCache(boolean clearReadPos, boolean clearCollect) {
        try {
            // 删除内存缓存
            String cacheDir = BaseApplication.getInstance().getApplicationContext().getCacheDir().getPath();
            ...
            // 清除其他缓存
            ACache.get(BaseApplication.getInstance().getApplicationContext()).clear();
        } catch (Exception e) {
            LogUtils.e(e.toString());
        }
    }
